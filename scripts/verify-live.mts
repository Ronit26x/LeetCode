...
console.log(`settings ok (tz ${settings.timezone}, start ${settings.dayStartHour}, interview ${settings.interviewDate}); tags: ${tagCount}`);

const slug = `verify-${randomUUID().slice(0, 8)}`;
const [p] = await db.insert(problems).values({ slug, title: "Verification problem", difficulty: "medium" }).returning();
const now = new Date();
...
const grade = await applyGrade(db, settings, { problemId: p.id, clientReviewId: randomUUID(), rating: 3, mode: "revise", durationSeconds: null, note: null, appendNoteToPitfalls: false }, later);
const dup = await applyGrade(db, settings, { problemId: p.id, clientReviewId: randomUUID(), rating: 3, mode: "revise", durationSeconds: null, note: null, appendNoteToPitfalls: false }, later);
console.log(`first solve ok; preview good=${preview[3]}d; grade scheduled=${grade.scheduledDays}d duplicate=${grade.duplicate}; second grade duplicate=${dup.duplicate}`);
...
console.log(`undo ok: reviseCount=${row?.reviseCount} resolveCount=${row?.resolveCount} reps=${row?.card?.reps} logs=${logs.length} undone=${logs.filter((l) => l.undoneAt).length}`);
