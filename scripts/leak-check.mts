...
const { getProblem, listProblems, listTags, listTagsWithCounts, searchProblemsBrief, countByStatus } = await import("@/lib/problems/queries");
...
async function step<T>(name: string, fn: () => Promise<T>): Promise<T> {
  const t0 = Date.now();
  const timer = new Promise<never>((_, rej) => setTimeout(() => rej(new Error(`HANG in step "${name}"`)), 20_000));
...
const [p] = await step("insert", () => db.insert(problems).values({ slug, title: "Leak check", difficulty: "easy" }).returning());
const now = new Date();
await step("first solve (tx)", () => db.transaction((tx) => applyFirstSolve(tx, p.id, 3, now, settings)));
...
const g = await step("grade (tx)", () => applyGrade(db, settings, { problemId: p.id, clientReviewId: randomUUID(), rating: 1, mode: "revise", durationSeconds: null, note: null, appendNoteToPitfalls: false }, later));
