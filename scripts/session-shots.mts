...
for (const [vpName, vp] of Object.entries(viewports)) {
  const context = await browser.newContext({ viewport: vp, deviceScaleFactor: 2, isMobile: vpName === "phone", hasTouch: vpName === "phone" });
  ...
  for (const theme of themes) {
    ...
    await page.screenshot({ path: `screenshots/${theme}-${vpName}-session-front.png`, fullPage: true });
    await page.locator("body").click({ position: { x: 5, y: 5 } });
    await page.keyboard.press("Space");
    await page.waitForTimeout(700);
    await page.screenshot({ path: `screenshots/${theme}-${vpName}-session-back.png`, fullPage: true });
