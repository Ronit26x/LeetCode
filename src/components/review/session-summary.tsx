...
export function SessionSummary({ stats, againTitles }: { stats: QueueStats; againTitles: { id: string; title: string }[] }) {
  const d = stats.doneToday;
  return (
    <div className="mx-auto max-w-[680px]">
      <div className="rounded-xl border border-border bg-surface p-6 sm:p-8">
        <h1 className="display text-2xl leading-8">{d.total ? "Session complete" : "Nothing due"}</h1>
        ...
        {againTitles.length ? (
          <div className="mt-5">
            ...
            <ul className="mt-1 flex flex-col gap-1 text-sm">
              {againTitles.map((a) => (
                <li key={a.id}>
                  <Link href={`/problems/${a.id}`} className="hover:underline underline-offset-2">
                    {a.title}
                  </Link>
                </li>
              ))}
        ...
        <p className="mt-5 text-md text-fg-muted">{pluralize(stats.tomorrow, "card")} due tomorrow.</p>
