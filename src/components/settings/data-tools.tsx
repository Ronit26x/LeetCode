...
export function DataTools() {
  ...
  function apply() {
    ...
    start(async () => {
      ...
      toast.success(
        `Imported: ${res.data.problems.create} new, ${res.data.problems.update} updated, ${res.data.logs.create} new logs`,
      );
      setFile(null);
      setPreview(null);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-2xs text-fg-subtle">
        Everything: problems, snippets, tags, cards, review logs, settings. Import is a dry run
        first, then idempotent by slug, tag name and review id.
      </p>
      {file && preview ? (
        <div className="rounded-md border border-border bg-sunken/50 p-3 text-md">
          ...
          <dl className="mt-2 grid grid-cols-2 gap-x-6 gap-y-1 text-2xs sm:grid-cols-3">
            <div className="flex justify-between">
              <dt className="text-fg-subtle">New problems</dt>
              <dd>{preview.problems.create}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-fg-subtle">Updated problems</dt>
              <dd>{preview.problems.update}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-fg-subtle">Cards</dt>
              <dd>{preview.cards}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-fg-subtle">New logs</dt>
              <dd>{preview.logs.create}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-fg-subtle">Logs already present</dt>
              <dd>{preview.logs.skip}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-fg-subtle">New tags</dt>
              <dd>{preview.tags.create}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-fg-subtle">Snippets</dt>
              <dd>{preview.snippets}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-fg-subtle">Relations</dt>
              <dd>{preview.relations}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-fg-subtle">Settings</dt>
              <dd>{preview.settings ? "yes" : "no"}</dd>
            </div>
          </dl>
