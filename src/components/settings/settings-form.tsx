    ...
    <div className="flex flex-col gap-6">
      <section className="rounded-xl border border-border bg-surface px-5 py-2">
        ...
        <div className="divide-y divide-border">
          ...
          <Row
            label="Day starts at"
            hint="Before this hour it is still yesterday's review day, like Anki."
            htmlFor="dayStart"
          >
            <NativeSelect
              id="dayStart"
          ...
          <Row
            ...
            hint="0.90 is the default. Workload rises steeply above 0.95."
            htmlFor="retention"
          >
            <NumberField
              id="retention"
              ...
              max={0.97}
              step={0.01}
              onCommit={(v) => v !== null && save({ desiredRetention: v })}
            />
          </Row>
          <Row label="Maximum interval" htmlFor="maxIvl">
            <NumberField
              ...
              max={36500}
              suffix="days"
              onCommit={(v) => v !== null && save({ maximumInterval: v })}
            />
          </Row>
          <Row
            label="Daily soft cap"
            hint="Hides the tail of Today behind Show more. Overdue cards are never hidden. Empty means unlimited."
            htmlFor="cap"
          >
            <NumberField
              id="cap"
              value={settings.dailySoftCap}
              min={1}
              max={1000}
              allowEmpty
              suffix="cards"
              onCommit={(v) => save({ dailySoftCap: v })}
            />
          </Row>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-surface px-5 py-2">
        ...
        <div className="divide-y divide-border">
          <Row
            ...
            hint="Drives the readiness panel, the retention ramp and the cram window. Clear it after the interview."
            htmlFor="interview"
          >
            <input
              id="interview"
          ...
          <Row
            label="Retention ramp"
            hint="Raise desired retention linearly over the final days before the interview."
            htmlFor="ramp"
          >
            <Switch
              id="ramp"
              checked={settings.retentionRampEnabled}
              onCheckedChange={(v) => save({ retentionRampEnabled: v })}
            />
          </Row>
          <Row label="Ramp length" htmlFor="rampDays">
            <NumberField
              id="rampDays"
              value={settings.retentionRampDays}
              min={1}
              max={90}
              suffix="days"
              onCommit={(v) => v !== null && save({ retentionRampDays: v })}
            />
          </Row>
          <Row label="Ramp target" hint="Retention on interview day." htmlFor="rampTarget">
            <NumberField
              id="rampTarget"
              value={settings.retentionRampTarget}
              min={0.8}
              max={0.97}
              step={0.01}
              onCommit={(v) => v !== null && save({ retentionRampTarget: v })}
            />
          </Row>
          <Row
            label="Cram window"
            hint="Inside this window Today shows not-yet-due cards with the lowest predicted recall, separately."
            htmlFor="cram"
          >
            <NumberField
              id="cram"
              value={settings.cramWindowDays}
              min={0}
              max={90}
              suffix="days"
              onCommit={(v) => v !== null && save({ cramWindowDays: v })}
            />
          </Row>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-surface px-5 py-2">
        ...
        <div className="divide-y divide-border">
          <Row
            label="Allow Easy in Revise"
            hint="Off by default: Easy is earned by resolving."
            htmlFor="easy"
          >
            <Switch
              id="easy"
              checked={settings.allowEasyInRevise}
              onCheckedChange={(v) => save({ allowEasyInRevise: v })}
            />
          </Row>
          <Row
            label="Resolve milestones"
            hint="Suggest a cold re-solve when stability first crosses each of these."
            htmlFor="milestones"
          >
            <input
              id="milestones"
              ...
              onBlur={() => {
                ...
                if (unique.join() !== settings.resolveMilestonesDays.join())
                  save({ resolveMilestonesDays: unique });
              }}
              className={cn(inputClass, "h-8 w-40 text-md")}
            />
          ...
          <Row
            label="Resolve after"
            hint="Consecutive revises before a resolve is suggested. 0 turns this rule off."
            htmlFor="afterN"
          >
            <NumberField
              id="afterN"
              value={settings.resolveAfterNRevises}
              min={0}
              max={50}
              suffix="revises"
              onCommit={(v) => v !== null && save({ resolveAfterNRevises: v })}
            />
          </Row>
          <Row
            label="Resolve time targets"
            hint="Timer presets and the time estimate on Today."
            htmlFor="tEasy"
          >
              ...
              {(["easy", "medium", "hard"] as const).map((d) => (
                <label key={d} className="flex items-center gap-1 text-2xs text-fg-subtle">
                  ...
                  <NumberField
                    ...
                    width="w-16"
                    onCommit={(v) =>
                      v !== null && save({ resolveTimeTargetsMin: { ...targets, [d]: v } })
                    }
                  />
                </label>
              ))}
          ...
          <Row
            ...
            hint="Minutes per revise, for the Today estimate."
            htmlFor="revEst"
          >
            <NumberField
              id="revEst"
              value={settings.reviseTimeEstimateMin}
              min={1}
              max={120}
              suffix="min"
              onCommit={(v) => v !== null && save({ reviseTimeEstimateMin: v })}
            />
          </Row>
        </div>
      </section>
