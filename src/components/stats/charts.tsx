...
export function StatTile({
  ...
  return (
    <div className={cn("rounded-xl border border-border bg-surface px-4 py-3", className)}>
      ...
      <p className="mt-1 text-2xl leading-8 font-semibold text-foreground [font-variant-numeric:proportional-nums]">
        {value}
      </p>
...
export function Heatmap({ data, tz }: { data: StatsData["heatmap"]; tz: string }) {
  ...
  for (const c of cells) {
    ...
    if (m !== lastMonth && c.row === 0) {
      months.push({
        col: c.col,
        label: formatInTimeZone(new Date(c.day + "T12:00:00Z"), "UTC", "MMM"),
      });
      lastMonth = m;
    }
  }
  ...
  for (const d of data)
    byMonth.set(d.day.slice(0, 7), (byMonth.get(d.day.slice(0, 7)) ?? 0) + d.count);
  return (
    <div>
      <div className="overflow-x-auto">
        <svg
          ...
          className="block"
          role="img"
          aria-label={`Reviews per day over the last year: ${total} reviews`}
        >
            ...
            {[...byMonth.entries()].map(([m, n]) => (
              <tr key={m}>
                <td className="pr-4 text-fg-muted">
                  {formatInTimeZone(new Date(m + "-15T12:00:00Z"), "UTC", "MMM yyyy")}
                </td>
...
export function TagMastery({ rows }: { rows: StatsData["tagMastery"] }) {
  ...
  if (shown.length === 0)
    return <p className="text-sm text-fg-subtle">No active cards with tags yet.</p>;
  return (
    <ul className="flex flex-col gap-1.5">
      {shown.map((r) => (
        <li
          key={r.id}
          className="flex flex-wrap items-center gap-x-3 gap-y-1 text-md sm:grid sm:grid-cols-[minmax(0,10rem)_1fr_3rem_3.5rem]"
        >
          <Link
            href={`/problems?tag=${r.id}`}
            className="flex min-w-0 flex-1 items-center gap-2 underline-offset-2 hover:underline sm:flex-none"
          >
            <span
          ...
          <div
            className="order-last h-2 basis-full rounded-[2px] bg-sunken sm:order-none sm:basis-auto"
            role="img"
            aria-label={`${r.name}: mean recall ${formatPercent(r.meanR)}`}
            data-tip={`${r.name}: mean recall ${formatPercent(r.meanR)}, mean stability ${r.meanS !== null ? formatStability(r.meanS) : "–"}`}
          >
            <div
              className="h-full rounded-[2px] bg-primary"
              style={{ width: `${Math.round((r.meanR ?? 0) * 100)}%` }}
            />
          </div>
          <span className="text-right text-fg-muted">{formatPercent(r.meanR)}</span>
          <span className="text-right text-2xs text-fg-subtle">
            S {r.meanS !== null ? formatStability(r.meanS) : "–"}
          </span>
        </li>
      ))}
    </ul>
    ...
    <ul className="flex flex-col gap-2">
      {rows.map((r) => {
        ...
        return (
          <li
            key={r.difficulty}
            className="grid grid-cols-[4.5rem_1fr_8rem] items-center gap-3 text-md"
          >
            <DifficultyBadge difficulty={r.difficulty} plain />
            <div
              className="relative h-2 rounded-[2px] bg-sunken"
              data-tip={`${DIFFICULTY_LABEL[r.difficulty]}: ${r.avgMinutes !== null ? `${r.avgMinutes.toFixed(0)} min average over ${r.n}` : "no timed resolves"}, target ${r.target} min`}
            >
              {r.avgMinutes !== null ? (
                <div
                  className={cn("h-full rounded-[2px]", over ? "bg-hard" : "bg-primary")}
                  style={{ width: `${Math.min(100, avgW)}%` }}
                />
              ) : null}
              <div
            ...
            <span className="text-right text-2xs text-fg-muted">
              {r.avgMinutes !== null ? `${r.avgMinutes.toFixed(0)} min` : "–"}{" "}
              <span className="text-fg-subtle">/ {r.target} target</span>
            </span>
          </li>
        );
      })}
