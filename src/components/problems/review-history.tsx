...
const RATING_CLASS: Record<number, string> = { 0: "text-fg-subtle", 1: "text-again", 2: "text-hard", 3: "text-good", 4: "text-easy" };
          ...
          {logs.map((l) => (
            <li key={l.id} className={cn("flex flex-col gap-0.5 py-2", l.undoneAt && "opacity-50")}>
              <div className="flex items-center gap-2 text-md">
                ...
                <span className="text-2xs text-fg-subtle">{l.rating === 0 ? "" : MODE_LABEL[l.mode]}</span>
                <span className={cn("font-medium", RATING_CLASS[l.rating])}>{RATING_LABEL[l.rating]}</span>
                {l.rating > 0 ? <span className="ml-auto text-fg-subtle">{formatInterval(l.resultScheduledDays)}</span> : null}
              </div>
