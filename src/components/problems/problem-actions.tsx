      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-52">
          {status === "suspended" ? (
            <DropdownMenuItem
              disabled={pending}
              onClick={() => run("Unsuspended", () => unsuspendProblems([id]))}
            >
              <Play /> Unsuspend
            </DropdownMenuItem>
          ) : status === "active" ? (
            <DropdownMenuItem
              disabled={pending}
              onClick={() => run("Suspended", () => suspendProblems([id]))}
            >
              <PauseCircle /> Suspend
            </DropdownMenuItem>
          ) : null}
          ...
          {status !== "archived" ? (
            <DropdownMenuItem
              disabled={pending}
              onClick={() => run("Archived", () => archiveProblems([id]))}
            >
              <Archive /> Archive
            </DropdownMenuItem>
          ) : (
            <DropdownMenuItem
              disabled={pending}
              onClick={() => run("Restored", () => unsuspendProblems([id]))}
            >
              <Play /> Restore
            </DropdownMenuItem>
          )}
      ...
      <AlertDialog open={confirm === "reset"} onOpenChange={(o) => !o && setConfirm(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Memory state goes back to new and it becomes due now. The review history stays.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={confirm === "delete"} onOpenChange={(o) => !o && setConfirm(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              The card, snippets, notes and every review log go with it. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
