...
import { countByStatus, listProblems, listTags, type ListFilters, type SortKey } from "@/lib/problems/queries";
import { getSettings } from "@/db/bootstrap";

export const metadata: Metadata = { title: "Problems" };

const SORTS: SortKey[] = ["due", "stability", "retrievability", "difficulty", "lastReviewed", "revises", "resolves", "title", "number", "created"];
...
function parseFilters(sp: Record<string, string | string[] | undefined>): ListFilters {
  ...
  return {
    ...
    status: status === "backlog" || status === "active" || status === "suspended" || status === "archived" ? status : "all",
...
export default async function ProblemsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [items, tags, counts, settings] = await Promise.all([listProblems(filters), listTags(), countByStatus(), getSettings()]);
    ...
    <>
      ...
      {total === 0 ? (
        ...
        <>
          ...
          {items.length === 0 ? (
            <p className="rounded-md border border-border px-4 py-8 text-center text-sm text-fg-muted">Nothing matches these filters.</p>
          ) : (
