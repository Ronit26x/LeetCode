...
function revalidateAll() {
  for (const p of ["/settings", "/today", "/review", "/problems", "/stats", "/backlog"])
    revalidatePath(p);
}
...
export async function updateSettings(raw: SettingsPatch): Promise<ActionResult> {
  ...
  if (patch.retentionRampTarget !== undefined || patch.desiredRetention !== undefined) {
    ...
    if (target < base)
      return { ok: false, error: "The ramp target must be at least the desired retention." };
  }
  const db = await getDb();
  // A changed day boundary or timezone means the ramp pass should run again on the next request.
  const extra =
    patch.timezone !== undefined ||
    patch.dayStartHour !== undefined ||
    patch.retentionRampEnabled !== undefined ||
    patch.retentionRampDays !== undefined ||
    patch.retentionRampTarget !== undefined ||
    patch.interviewDate !== undefined
      ? { lastRampAppliedDay: null }
      : {};
  await db
    .update(settings)
...
export async function updateFsrsWeights(raw: unknown): Promise<ActionResult> {
  ...
  await db
    .update(settings)
    .set({ fsrsParams: parsed.data, updatedAt: new Date() })
    .where(eq(settings.id, 1));
  revalidateAll();
  return { ok: true, data: null };
}
...
export async function resetFsrsWeights(): Promise<ActionResult> {
  ...
  await db
    .update(settings)
    .set({ fsrsParams: DEFAULT_FSRS_W, updatedAt: new Date() })
    .where(eq(settings.id, 1));
  revalidateAll();
  return { ok: true, data: null };
}
